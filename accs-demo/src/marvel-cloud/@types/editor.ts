...
export default interface Editor {
   ...
   * @param path 必须是完成路径
   */
  addMediaToMainTrack(path: Path): ClipId;
  /**
   * 添加一条视频资源到指定轨道上（mp4/mov）
   * @param path 必须是完成路径
   * @param trackId ?
   */
  addMediaToTrack(path: Path, trackId?: TrackId): ClipId;
  /**
   * 添加一条图片资源到主轨上（jpg/png）
   * @param path 必须是完成路径
   * @param stopTimeUS 展示的时长，将追加在主轨最后
   */
  addImageToMainTrack(path: Path, stopTimeUS: TimeUS): ClipId;
  /**
   ...
   */
  addImageToTrack(path: Path, stopTimeUS: TimeUS, trackId?: TrackId): ClipId;
  /**
   * 添加一条音频到新编辑轨道
   * @param path 必须是完成路径
   */
  addMusic(path: Path): ClipId;
  /**
   * 添加一条滤镜到新编辑轨道
   * @param path 必须是完成路径
   * @param intensity 效果阈值
   */
  addLookup(path: Path, intensity: Intensity): ClipId;
  /**
   * 添加一条贴纸到新编辑轨道
   * @param path 必须是完成路径
   * @param stopTimeUS 展示的时长
   */
  addSticker(path: Path, stopTimeUS: TimeUS): ClipId;
  /**
   * 添加一条特效到新编辑轨道
   * @param path 必须是完成路径
   * @param stopTimeUS 展示的时长
   ...
   * @param clipId
   */
  removeClip(clipId: ClipId): Ret;
  /**
   * 分割 Clip
   * @param timeUS
   */
  splitClip(timeUS: TimeUS): ClipId[];
  /**
   * 移动 clip, 先使用 selectClip 选择当前的 clip
   * @param startTimeUs
   * @param trackId
   */
  moveClip(startTimeUs: TimeUS, trackId: TrackId): Ret;
  /**
   * 修改当前 clip 音量 先使用 selectClip 选择当前的 clip
   * @param volume 0.000000 - 1.000000 范围
   */
  changeVolume(volume: number): Ret;

  /**
   * 修改当前Clip的播放速度
   * @param speed 0.000000 - 1.000000 范围
   */
  changeSpeed(speed: number): Ret;
  /**
   ...
   * @param stopUs
   * */
  changeSourceTime(startUs: TimeUS, stopUs: TimeUS): Ret;
  /**
   ...
   * @param stopUs
   */
  changePlayTime(startUs: TimeUS, stopUs: TimeUS): Ret;
  /**
   ...
   * @param trackId
   */
  getTrackInfo(trackId: TrackId): TrackInfo;
  /**
   ...
   * @param type
   */
  getTinyClipInfo(clipId: ClipId): ClipInfo<BaseClipInfo>;
  /**
   * 获取指定 Clip 信息，type来获取各种效果，滤镜的信息操作。
   * @param clipId
   * @param type
   */
  getClipInfo<T extends MarvelClipInfo>(clipId: ClipId, type?: ClipType | string): ClipInfo<T>;
  /**
